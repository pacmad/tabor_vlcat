...
if ( file_exists( "../../../promenne.php" ) && require( "../../../promenne.php" ) )
{
	if ( ($spojeni = mysqli_connect( $db_host, $db_username, $db_password, $db_name ) ) && $spojeni->query("SET CHARACTER SET UTF8") )
	{
		...
		$suma_celk_karta = 0;
		$suma_celk_hot = 0;
		$prikaz_karta = 'SELECT * FROM '.$countTable.' WHERE typ = "karta"';
		$prikaz_hot = 'SELECT * FROM '.$countTable.' WHERE typ = "hotovost"';
		...
		$sql = $spojeni->query("SELECT checked, value FROM ".$checkTable." WHERE id = (SELECT max(id) FROM ".$checkTable." WHERE typ = 'karta')");
		$toto1 = mysqli_fetch_array( $sql );
		$sql = $spojeni->query("SELECT checked, value FROM ".$checkTable." WHERE id = (SELECT max(id) FROM ".$checkTable." WHERE typ = 'hotovost')");
		$toto2 = mysqli_fetch_array( $sql );
		
		echo '<em>Zůstatek na kartě: 
					<button name="check_K" onclick="updateState( \'karta\', \''.$checkTable.'\', \'prostred\', \''.number_format((float)$zustatek_karta, 2, '.', '').'\', \''.$who.'\' )" class="'.$redK.'"><strong class="check_H">'.number_format((float)$zustatek_karta, 2, ',', ' ').' '.$currency.'</strong></button>
			  </em><span title="'.number_format((float)$toto1['value'], 2, ',', ' ').'">'.dateToReadableFormat($toto1['checked']).'</span><br />';
		echo '<em>Zůstatek hotovosti: 
					<button name="check_H" onclick="updateState( \'hotovost\', \''.$checkTable.'\', \'prostred\', \''.number_format((float)$zustatek_hot, 2, '.', '').'\', \''.$who.'\' )" class="'.$redH.'"><strong class="check_H">'.number_format((float)$zustatek_hot, 2, ',', ' ').' '.$currency.'</strong></button>
